<?php
/**
 * Sarada Lite Customizer Notification System.
 *
 * @package Sarada_Lite
 */
 ...
 */
class Sarada_Lite_Customizer_Notice {
	...
	/**
	 * The single instance of Sarada_Lite_Customizer_Notice
	 *
	 * @var Sarada_Lite_Customizer_Notice $instance The Sarada_Lite_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Sarada_Lite_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Sarada_Lite_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Sarada_Lite_Customizer_Notice ) ) {
			self::$instance = new Sarada_Lite_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $sarada_lite_recommended_plugins;
		...
		$sarada_lite_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$sarada_lite_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'sarada-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'sarada-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), SARADA_LITE_THEME_VERSION, true );
		wp_localize_script(
			'sarada-lite-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'sarada-lite' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Sarada_Lite_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Sarada_Lite_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Sarada_Lite_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'sarada_lite_show_recommended_plugins' );
			...
			update_option( 'sarada_lite_show_recommended_plugins', $show_recommended_plugins );
		}
