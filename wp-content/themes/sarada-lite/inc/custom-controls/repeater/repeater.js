/*jshint -W065 */
var saradaliteRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['sarada-lite-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new saradaliteRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'sarada_lite_get_fontawesome_ajax',
					sarada_lite_customize_nonce: sarada_lite_customize.nonce
				},
				beforeSend: function(){
